...
class SAC:
    ...
    def update(self, transition_dict):
        ...
        # 对states_fc进行归一化
        # c = 0.001
        # a-0.0001、a-1e-5  c-0.001使用  a-0.001 c-0.001不使用
        # states_fc = self.min_max_scaling_tensor(states_fc_)
        # 对states_conv进行归一化
        # states_conv = self.min_max_scaling_tensor(states_conv_)
        ...
        # # 防止数值溢出
        # critic_1_loss = torch.clamp(critic_1_loss, min=-1e15, max=1e15)
        # 反向传播
        ...
        # 防止数值溢出
        # critic_2_loss = torch.clamp(critic_2_loss, min=-1e15, max=1e15)
        ...
        # 防止数值溢出
        # actor_loss = torch.clamp(actor_loss, min=-1e15, max=1e15)
        # 更新策略网络
